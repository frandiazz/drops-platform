...
const badges = [
  { icon: 'check', title: 'SSL Seguro', desc: 'Cifrado AES-256 · Datos protegidos', color: 'text-green-400' },
  { icon: 'shield', title: 'DMCA Protegido', desc: 'Contenido registrado · Takedown activo', color: 'text-accent-violet' },
  { icon: 'mp', title: 'Verificado por Mercado Pago', desc: 'Tokenización · Cumplimiento PCI', color: 'text-blue-400' },
];
    ...
    <section id="seguridad" className="py-24 relative" aria-labelledby="security-heading">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-12 flex flex-wrap items-center justify-center gap-6 section-fade">
          {badges.map((badge, i) => (
            <div key={i} className="flex items-center gap-3 px-5 py-3 rounded-xl bg-dark-light/40 border border-slate-700/30">
              <svg className={`w-8 h-8 ${badge.color}`} fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z" /></svg>
              <div>
